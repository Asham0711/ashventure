...
const BlogDetailsPage = () => {
  ...
  const nextImage = () => {
    setCurrentIndex((prev) => (prev === blog.images.length - 1 ? 0 : prev + 1));
  };

  const prevImage = () => {
    setCurrentIndex((prev) => (prev === 0 ? blog.images.length - 1 : prev - 1));
  };
    ...
    return Array.from({ length: 5 }, (_, index) => {
      ...
      return (
        <Star
          ...
          size={20}
          className={
            isFilled ? "fill-yellow-400 text-yellow-400" : "text-white"
          }
        />
      );
    });
  ...
  return (
    <div className="max-w-screen min-h-screen  mx-auto overflow-x-hidden w-full h-full">
      {isLoading && <Loader text="Loading blog..." fullPage />}
        ...
        <>
          <div className="lg:w-9/12 w-11/12 mx-auto mt-16 md:mt-24 flex flex-col justify-center items-center gap-4">
            <h1 className="text-3xl lg:text-5xl text-center">
              Travel Stories That Inspire <HighlightText text="Adventures" />
            </h1>
            <p className="text-base md:text-xl text-center hidden md:block">
              Get detailed insights about destinations, trip duration, travel
              styles, budgets, and honest ratings — everything you need to plan
              your next journey.
            </p>
          </div>
          <div className="md:w-11/12 w-[95%] mx-auto bg-transparent backdrop-blur-sm border border-white/20 md:py-8 md:px-4 py-4 px-2 rounded-3xl my-8 space-y-4 relative">
            <h1 className="md:text-3xl text-lg mb-4 text-primary-brand text-center">
              {blog.title}
            </h1>
            <div className="flex items-center justify-center gap-2">
              {renderStars(blog.rating)}
              <span className="text-sm text-white">{blog.rating}/5</span>
            </div>
            <div className="w-11/12 mx-auto text-center md:text-base text-sm">
              Published on {new Date(blog.createdAt).toDateString()}, this
              in-depth travel experience showcases a{" "}
              {blog.travelType.toLowerCase()} getaway to {blog.placeName}. Over
              the course of {blog.tripDuration} days in {blog.travelMonth}, the
              trip was thoughtfully managed within a{" "}
              {blog.budgetRange.toLowerCase()} budget, delivering both
              inspiration and practical guidance for fellow travelers.
            </div>
            ...
            <div className="md:text-sm text-xs text-gray-200 font-serif absolute right-3 bottom-3">
              <p>
                {blog.userId?.firstName} {blog.userId?.lastName} (Author)
              </p>
